...
###########################################
## Retain the first 3 Kb from TSS (start) # 20220329
###########################################
...
for(i in 1:nrow(refseq)) {
  if(refseq[i,6] == '+') {
    ...
    refseq[i,8]=(refseq[i,2]+3000)
  }
  else if(refseq[i,6] == '-'){
    refseq[i,7]=(refseq[i,3]-3000)
...
aa
refseq <- refseq[!refseq$Start < 0,] #1 gene
nrow(refseq)
# -----------
# Save data |
# -----------
write.table(refseq, 
            file = paste0(output,"RefSeq_LongList_TSS_3Kb.bed"),
            quote = F, 
            sep="\t", 
            col.names = F, 
            row.names = F)